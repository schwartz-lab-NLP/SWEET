...
from transformers.utils.temperature_scaling import set_temperature
...
class DataTrainingArguments:
    ...
    extract_lr: bool = field(default=False, metadata={"help": "A boolean flag indicating if to extract lr."})
...
class ModelArguments:
    ...
    early_exit: bool = field(
        default=False,
        metadata={"help": "Boolean flag indicating standard EE fine tuning method"},
    )
...
def main():
    ...
    if data_args.extract_lr:
        training_args.output_dir = training_args.output_dir if training_args.output_dir.startswith(path.sep) else path.sep + training_args.output_dir
        lr_dir = path.sep + path.join(*training_args.output_dir.split(path.sep)[:6], 'lr_output', *training_args.output_dir.split(path.sep)[7:-1])
        for dir in os.listdir(lr_dir):
            if dir.startswith('best'):
                training_args.learning_rate = float(dir.split('_')[-1])
                print('extracted_lr_successfully')
                break
    ...
    exit_thresholds = parse_exit_thresholds(int(num_labels), int(model_args.exit_thresholds), model_args.exit_strategy=='lte')
    ...
    configs = [AutoConfig.from_pretrained(
            ...
            exit_kwargs = exit_kwargs) for exit_layer_ind in exit_layers]
    ...
    if training_args.do_eval:
        ...
        for model in models:
            model.eval()
        layer_predictions = np.zeros((data_shape, len(models),1, num_labels))
        should_early_exit = np.zeros((len(exit_thresholds),data_shape, len(models)))
        label_ids = np.zeros((data_shape, 1))
        # Instead of evaluating for each threshold, get all logits for all models and calculate speedups later.
        for batch_i, batch in enumerate(eval_dataloader):
            label_ids[batch_i, ...] = batch['labels'][0]
            batch = move_to_device(batch, training_args.device)
            for model_i, model in enumerate(models):
                model = model.to(training_args.device)
                with torch.no_grad():
                    outputs = model(**batch, output_hidden_states = True)
                layer_predictions[batch_i, model_i, ...] = outputs.logits.cpu()
                exit_strategy_logits = outputs.logits.squeeze(0)
                if model_args.exit_strategy == 'lte':
                    exit_strategy_logits = outputs.hidden_states[-1].cpu()
                for thr_i, thr in enumerate(exit_thresholds):
                    model.set_exit_strategy(model_args.exit_strategy, threshold = thr)
                    should_early_exit[thr_i,batch_i,model_i] = model.exit_strategy(exit_strategy_logits)
            batch = move_to_device(batch, 'cpu')

        for thr_i, thr in enumerate(exit_thresholds):
            total_predictions = np.zeros((data_shape, len(models), num_labels))
            for pred_i, prediction in enumerate(layer_predictions):
                was_predicted = should_early_exit[thr_i, pred_i].any()
                predicting_model = np.argmax(should_early_exit[thr_i,pred_i,:]) if was_predicted else (len(models) -1)
                total_predictions[pred_i, :predicting_model + 1 , ...] = prediction[:predicting_model + 1, ...].squeeze(1)
